...
char lastValue = '9';   //speichert den letzten Status der LED (on/off) 
MFRC522 mfrc522;
  ...
  if(Serial.available()) //wenn Daten empfangen werden...      
{
  ...
  {           
    ...
    if (lastValue != '0' && blueToothVal == '0'){  
      LED_blink(red_LED); 
    } 
    lastValue=blueToothVal;
  }
  else if (blueToothVal=='9') //wenn das Bluetooth Modul „9“ empfängt...
  {    
...
void RFIDInformation(){
  ...
  content.toUpperCase();
  String id = "CardID " + content.substring(1);
  Serial.println();
  delay(50);
  Serial.print(id);
  /*
  if(content.substring(1) == "76 C5 C3 3B"){
    //Serial.println("Authorized access");
    //Serial.println();
    LED_blink(green_LED);
  } else{
    //Serial.println("Authorized denied! Please register your card.");
    //Serial.println();
    LED_blink(red_LED);
  } */
}
